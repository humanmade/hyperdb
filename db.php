...
class hyperdb extends wpdb {
	...
	function is_write_query( $q ) {
		...
		// Fix by Joe Hoyle to treat SELECT ... FOR UPDATE queryies as writes
		// @see http://wordpress.org/support/topic/select-for-update-is-sent-to-read-server-when-it-shouldnt-be
		if ( strpos( $q, 'FOR UPDATE' ) ) {
			return true;
		}
